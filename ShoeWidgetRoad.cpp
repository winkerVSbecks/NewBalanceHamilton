...
{
  ShoeWidgetRoad::ShoeWidgetRoad(MultiWidgets::Widget * parent)
      : Widget(parent)
  {
      slideOn = false;
  }

  ShoeWidgetRoad::~ShoeWidgetRoad()
  {}

  void ShoeWidgetRoad::setupTheShoeWidgetRoad(float x, float y, float rot)
  {
      bg = new MultiWidgets::ImageWidget(this);
      bg->load("shoeWidget/shoeWidgetRoad.png");
      ...
      // Initial location
      sl->setLocation(0,0);
      sl->makeSlider(false,"minimus-Road");
      sl->hide();
  }

  void ShoeWidgetRoad::input(MultiWidgets::GrabManager & gm, float dt)
 {
   Widget::input(gm, dt);
  ...
  void ShoeWidgetRoad::processMessage(const char * id, Radiant::BinaryData & data)
      {
        if(strcmp(id, "die") == 0) {
            raiseFlag(DELETE_ME);
  ...
  void ShoeWidgetRoad::update(float dt)
  {
      Widget::update(dt);
  ...
  CUSTOM_WIDGET(ShoeWidgetRoad, "queue.shoewidgetroad", "winkerVSbecks",
                "Shoe Widget Road", "NewBalance");
}
