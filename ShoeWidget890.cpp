...
{
  ShoeWidget890::ShoeWidget890(MultiWidgets::Widget * parent)
      : Widget(parent)
  {
      slideOn = false;
  }

  ShoeWidget890::~ShoeWidget890()
  {}

  void ShoeWidget890::setupTheShoeWidget890(float x, float y, float rot)
  {
      bg = new MultiWidgets::ImageWidget(this);
      bg->load("shoeWidget/shoeWidget890.png");
      ...
      // Initial location
      sl->setLocation(0,0);
      sl->makeSlider(false, "890");
      sl->hide();
  }

  void ShoeWidget890::input(MultiWidgets::GrabManager & gm, float dt)
 {
   Widget::input(gm, dt);
  ...
  void ShoeWidget890::processMessage(const char * id, Radiant::BinaryData & data)
      {
        if(strcmp(id, "die") == 0) {
            raiseFlag(DELETE_ME);
  ...
  void ShoeWidget890::update(float dt)
  {
      Widget::update(dt);
  ...
  CUSTOM_WIDGET(ShoeWidget890, "queue.shoewidget890", "winkerVSbecks",
                "Shoe Widget 890", "NewBalance");
}
