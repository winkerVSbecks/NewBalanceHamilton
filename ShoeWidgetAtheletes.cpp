...
#include <MultiWidgets/Flipping.hpp>
...
{
  ShoeWidgetAtheletes::ShoeWidgetAtheletes(MultiWidgets::Widget * parent)
      : Widget(parent)
  {
      slideOn = false;
  }

  ShoeWidgetAtheletes::~ShoeWidgetAtheletes()
  {}

  void ShoeWidgetAtheletes::setupTheShoeWidgetAtheletes(float x, float y, float rot)
  {
      bg = new MultiWidgets::ImageWidget(this);
      bg->load("shoeWidget/shoeWidgetAtheletes.png");
      ...
      // Initial location
      sl->setLocation(0,0);
      sl->makeSlider(true, "Athletes");
      sl->hide();
  }

  void ShoeWidgetAtheletes::input(MultiWidgets::GrabManager & gm, float dt)
 {
   Widget::input(gm, dt);
  ...
  void ShoeWidgetAtheletes::processMessage(const char * id, Radiant::BinaryData & data)
      {
        if(strcmp(id, "die") == 0) {
            raiseFlag(DELETE_ME);
  ...
  void ShoeWidgetAtheletes::update(float dt)
  {
      Widget::update(dt);
  ...
  CUSTOM_WIDGET(ShoeWidgetAtheletes, "queue.shoewidgetatheletes", "winkerVSbecks",
                "Shoe Widget Atheletes", "NewBalance");
}
